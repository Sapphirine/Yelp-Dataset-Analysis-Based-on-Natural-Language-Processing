...
function generateMap(apiPath, containerId) {
     ...
     * @class Marker
     */
    var Marker = function Marker(name, stars, address, loc) {
    ...
    function collectMarkers(data) {
        ...
        data.events.forEach(function (event) {
            ...
            pureLocations.forEach(function (location) {
                markers.push(new Marker(event.name, event.stars, event.address, location));
            });
        });
    ...
    function renderMarkers(markers) {
        markers.forEach(function (marker) {
            ...
            google.maps.event.addListener(m, 'click', function () {
                ...
                var a = 3.4835
                info.setContent('<b>Name:</b> ' + marker.name + '<br>' + '<b>Stars:</b> ' + marker.stars + '<br>' +'<b>Overall Rating:</b>'+ a +'<br>'+ '<b>Address:</b> ' + marker.address + '<br>');
                info.open(map, this);
            });
        });
    }
     ...
     */
    function renderEvent(addr, name) {
        geocoder.geocode({
            'address': addr
        }, function (res, status) {
            console.log(status);
            if (status == 'OK') {
                ...
                google.maps.event.addListener(m, 'click', function () {
                    var info = new google.maps.InfoWindow();
                    info.setContent('<b>Name:</b> ' + name + '<br>' + '<b>Address:</b> ' + addr + '<br>');
                    info.open(map, this);
                });
            } else {
        ...
        function renderData(data, showLeads, showEvents, showHeatmap) {
            ...
            if (showEvents) {
                parsedData.events.forEach(function (event) {
                    renderEvent(event.address, event.name);
                });
            }
